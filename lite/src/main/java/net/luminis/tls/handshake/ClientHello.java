...
public class ClientHello extends HandshakeMessage {
    ...
    public static final List<TlsConstants.CipherSuite> SUPPORTED_CIPHERS = Collections.singletonList(TlsConstants.CipherSuite.TLS_AES_128_GCM_SHA256);
    private static final int MAX_CLIENT_HELLO_SIZE = 3000;
    private static final int MINIMAL_MESSAGE_LENGTH = 1 + 3 + 2 + 32 + 1 + 2 + 2 + 2 + 2;
    private static final List<TlsConstants.SignatureScheme> SUPPORTED_SIGNATURES = Collections.singletonList(TlsConstants.SignatureScheme.rsa_pss_rsae_sha256);
    ...
    public ClientHello(String serverName, PublicKey publicKey, boolean compatibilityMode, List<TlsConstants.CipherSuite> supportedCiphers,
        ...
        extensions.addAll(Arrays.asList(defaultExtensions));

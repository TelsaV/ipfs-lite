...
public class KadDht implements Routing {
            ...
            try {
                ...
                for (String multiAddress : addresses) {
                    try {
                        ...
                        if (host.addToAddressBook(peerId, result, false)) {
    ...
    private Set<PeerId> evalClosestPeers(@NonNull Dht.Message pms) {
        ...
        for (Dht.Message.Peer entry : list) {
            ...
            if (host.addToAddressBook(peerId, multiAddresses, false)) {
                peers.add(peerId);
            } else {
                ...
                for (Dht.Message.Peer entry : list) {
                    ...
                    if (host.hasAddresses(peerId)) {
                        LogUtils.debug(TAG, "findProviders " + peerId.toBase58() + " "
                                + host.getAddresses(peerId).toString() + " for " +
                                cid.String() + " " + cid.getVersion());
    ...
    private void sendMessage(@NonNull Closeable closeable, @NonNull PeerId peerId,
        ...
        try {
            conn = host.connect(closeable, peerId, IPFS.CONNECT_TIMEOUT, IPFS.GRACE_PERIOD,
    ...
    private void sendMessage(@NonNull QuicClientConnection conn, @NonNull Dht.Message message) {
        ...
        try {
            ...
            kadDhtSend.writeAndFlush(DataHandler.writeToken(IPFS.STREAM_PROTOCOL, IPFS.DHT_PROTOCOL));
    ...
    private Dht.Message sendRequest(@NonNull Closeable closeable, @NonNull PeerId peerId,
        ...
        QuicClientConnection conn = host.connect(closeable, peerId, IPFS.CONNECT_TIMEOUT,
                IPFS.GRACE_PERIOD, IPFS.MIN_STREAMS, IPFS.DHT_STREAM_SIZE_LIMIT);
        ...
        try {
            ...
            dhtRequest.writeAndFlush(DataHandler.writeToken(IPFS.STREAM_PROTOCOL, IPFS.DHT_PROTOCOL));
    ...
    public void findPeer(@NonNull Closeable closeable, @NonNull Updater updater, @NonNull PeerId id) {
        ...
        try {
            runLookupWithFollowup(closeable, key, (ctx, p) -> {
                ...
                for (PeerId peerId : peers) {
                    if (Objects.equals(peerId, id)) {
                        LogUtils.debug(TAG, "findPeer " + host.getAddresses(peerId).toString());
                        updater.peer(peerId);
                    }
                }
            ...
            LogUtils.info(TAG, "Finish findPeer at " + (System.currentTimeMillis() - start));
        }
    }

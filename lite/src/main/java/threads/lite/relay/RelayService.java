...
public class RelayService {
    ...
    public static boolean canHop(@NonNull QuicClientConnection conn) {
    ...
    public static QuicStream getStream(@NonNull QuicClientConnection quicClientConnection,
                                       @NonNull PeerId self,
                                       @NonNull PeerId peerId, int timeout) throws IOException, ProtocolIssue, DataLimitIssue {
        ...
        RelayRequest relayRequest = new RelayRequest(quicStream, timeout,
                TimeUnit.SECONDS);
        ...
        if (msg.getType() != Relay.CircuitRelay.Type.STATUS) {
            relayRequest.closeOutputStream();
            throw new ConnectionIssue("no relay stream success");
        }

...
public class BitSwapSend {
    ...
    public BitSwapSend(@NonNull QuicStream quicStream, long readTimeout, TimeUnit unit) {
    ...
    public void reading() throws IOException, ProtocolIssue, DataLimitIssue {
        DataHandler reader = new DataHandler(new HashSet<>(
                Arrays.asList(IPFS.STREAM_PROTOCOL, IPFS.BITSWAP_PROTOCOL)
        ...
        while ((length = inputStream.read(buf, 0, 4096)) > 0) {
            ...
            if (reader.isDone()) {
                if (reader.getTokens().contains(IPFS.BITSWAP_PROTOCOL)) {
                    return;
                }
            }
        }

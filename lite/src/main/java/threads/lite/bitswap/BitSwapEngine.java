        ...
        for (BitSwapMessage.Entry entry : wants) {
            ...
            if (blockSize == null) {
                ...
                if (IPFS.SEND_DONT_HAVES && entry.SendDontHave) {
                    ...
                    if (!msg.Empty()) {
                        try {
                            bitSwap.writeMessage(new TimeoutCloseable(IPFS.CONNECT_TIMEOUT), peer,
                                    msg, IPFS.CONNECT_TIMEOUT, IPFS.PRIORITY_NORMAL);
                ...
                if (!msg.Empty()) {
                    try {
                        bitSwap.writeMessage(new TimeoutCloseable(IPFS.CONNECT_TIMEOUT), peer,
                                msg, IPFS.BITSWAP_GRACE_PERIOD, IPFS.PRIORITY_NORMAL);
                    } catch (Throwable throwable) {
                        LogUtils.error(TAG, throwable);
                    }
                }
